...
1. [Create an account or log in to IBM Cloud](http://bluemix.net)
1. [Create a Cloudant instance in your dashboard](https://console.bluemix.net/docs/services/Cloudant/tutorials/create_service.html#creating-a-cloudant-nosql-db-instance-on-ibm-cloud)

To create a local instance in a Docker container:
1. Use the [Cloudant developer image on Dockerhub](https://hub.docker.com/r/ibmcom/cloudant-developer/)
...
Note: if you are using Docker the `cloudant.username` and `cloudant.password` can be omitted and `cloudant.url` can be set to `http://localhost:8080`.
